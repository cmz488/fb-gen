...
impl FffScanner {
    ...
    /// `#include "..."` directives.
    pub fn scan_source_files(&self, options: &ScanOptions) -> Result<Vec<SourceFile>> {
    ...
    pub fn scan_includes(&self, file_path: &Path) -> Result<Vec<String>> {
    ...
    pub fn scan_single(&self, file_path: &Path) -> Result<SourceFile> {
    ...
    pub fn scan_presets(&self, root: &Path) -> Result<Option<CMakePresets>> {
